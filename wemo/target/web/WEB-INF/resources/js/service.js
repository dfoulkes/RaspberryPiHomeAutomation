...
homeAutomationApp.factory('ComponentService', function() {
    ...
    srv.getByIdComponent = function(id) {
        //
        $.ajax({
            url: "/core/getAllComponents.json",
            async: false
    ...
    srv.add = function(com,ip,port){
        $.ajax({
            url: "/core//addCom.html?ip="+ip+"&type="+com+"&address="+port,
            async: false
        }).then(function(data) {
            srv._books = data;
        });
    ...
    return {
        ...
        add: function(com, ip, port) {
            return srv.add(com,ip, port);
        },
        getByIdComponent: function(id){
            return srv.getByIdComponent(id);
        }
    };
});
